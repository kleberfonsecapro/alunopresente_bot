...
class AlunoPresenteExtractor(SiteExtractor):
    ...
    async def extract_single_unit(
        ...
        data = {
            ...
            'ausentes': fmt_int(unit['expectativaPresenca'] - unit['alunos_presentes']),
            'presentes_pct': fmt_pct(unit['percentual_presenca']),
            'ausentes_pct': fmt_pct((unit['expectativaPresenca'] - unit['alunos_presentes']) / unit['expectativaPresenca'] * 100) if unit['expectativaPresenca'] else '0,0',
            'alimentados': fmt_int(unit['alunos_alimentados']),
            'nao_alimentados': fmt_int(unit.get('nao_alimentados', unit['alunos_presentes'] - unit['alunos_alimentados'])),
            'alimentados_pct': fmt_pct(unit['percentual_alimentados']),
            'nao_alimentados_pct': fmt_pct(unit.get('percentual_nao_alimentados', 100 - unit['percentual_alimentados'])),
    ...
    async def _fetch_api_data(self, headers: dict, unit_ids: list[int] | None = None, period: str | None = None) -> dict:
        ...
        total_ausentes = sum(u['expectativaPresenca'] - u['alunos_presentes'] for u in unit_list) if unit_list else 0
        total_nao_alimentados = sum(u.get('nao_alimentados', u['alunos_presentes'] - u['alunos_alimentados']) for u in unit_list) if unit_list else 0
        total_sem_foto = sum(u.get('alunos_sem_foto', u['total_alunos'] - u['alunos_com_foto']) for u in unit_list) if unit_list else 0
        ...
        data = {
            ...
            'ausentes': fmt_int(total_ausentes),
            'presentes_pct': fmt_pct(total_presentes / total_esperados * 100) if total_esperados else '0,0',
            'ausentes_pct': fmt_pct(total_ausentes / total_esperados * 100) if total_esperados else '0,0',
            'alimentados': fmt_int(total_alimentados),
            'nao_alimentados': fmt_int(total_nao_alimentados),
            'alimentados_pct': fmt_pct(total_alimentados / total_presentes * 100) if total_presentes else '0,0',
            'nao_alimentados_pct': fmt_pct(total_nao_alimentados / total_presentes * 100) if total_presentes else '0,0',
            'com_foto': fmt_int(total_com_foto),
            'sem_foto': fmt_int(total_sem_foto),
            'com_foto_pct': fmt_pct(total_com_foto / total_alunos * 100) if total_alunos else '0,0',
            'sem_foto_pct': fmt_pct(total_sem_foto / total_alunos * 100) if total_alunos else '0,0',
        }
    ...
    async def relogin(self) -> None:
        ...
        from aluno_presente_sme.skills.auth_skill import clear_storage_state
        import os
        clear_storage_state()
